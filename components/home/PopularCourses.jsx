      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <div className="flex flex-col gap-4 mb-7">
          ...
          <div className="relative border-b border-gray-200">
            <TabsList 
              ref={tabsListRef} 
              className="bg-transparent p-0 gap-8 md:gap-12 h-auto border-none flex-nowrap justify-start leading-none overflow-x-auto no-scrollbar"
            >
              {categories.map((cat) => (
                <TabsTrigger
                  key={cat}
                  value={cat}
                  className="rounded-none border-none bg-transparent data-[state=active]:shadow-none data-[state=active]:text-text-main px-0 py-3.5 text-base md:text-lg 2xl:text-2xl font-semibold text-[#808080] transition-colors hover:text-text-main z-10 leading-none after:hidden shrink-0 cursor-pointer"
                >
                  {cat}
                </TabsTrigger>
              ))}
